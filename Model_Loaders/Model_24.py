...
#                     return self.self_mlp(x) + self.msg_mlp2(torch.cat(tmp,0))
            ...
            for _ in range(N_metalayers):
                self.convs.append(GATConv(self.hcs,self.hcs,N_heads))
                self.scatter_norms.append(scatter_norm(self.hcs*N_heads))

            self.decoder = MLP([(1+N_scatter_feats*N_metalayers*N_heads)*self.hcs,self.hcs,N_outputs],clean_out=True)
...
#         def FC_edge_index(self,graph_node_counts):
#             li : List[int] = graph_node_counts.tolist()
#             starts = torch.cumsum(torch.cat([torch.zeros(1,dtype=torch.long,device=self.device),graph_node_counts],dim=0),dim=0)

#             edge_index = torch.cat([torch.combinations(torch.arange(start,start+length,device=self.device)) for start, length in zip(starts[:-1],graph_node_counts)],dim=0)
