...
#                     return self.basis_func(distances)
            ...
            class MLP(torch.nn.Module):
                def __init__(self, hcs_list, act = self.act, clean_out = False,p=0.1):
...
#                     self.lin_CoC_msg = MLP([self.hcs, self.hcs],clean_out = True)
                    self.lin_CoC_self = MLP([self.hcs, self.hcs],clean_out = True)
                    
#                     self.RBF = RBF(self.hcs, 4*self.hcs)
...
#                     msg = self.lin_CoC_msg( scatter_sum(self.RBF(h), batch, dim=0) )
...
#             CoC = torch.cat([CoC,scatter_sum(self.decoderRBF(x),batch,dim=0)],dim=1)
