        ...
        def __init__(self):
            ...
            class NodeModel(torch.nn.Module):
                def __init__(self,hcs,act):
                    ...
                    self.GATConv = GATConv(self.hcs, self.hcs, add_self_loops = False)
                ...
                def forward(self, x, edge_index, edge_attr, u, batch):
                    ...
                    x, (edge_index, attentions) = self.GATConv(x, edge_index, return_attention_weights = True)
                    
                    row, col = edge_index
                    out = torch.cat([x[row],edge_attr],dim=1)
                    for lin in self.lins1:
                        out = self.act(attentions*lin(out))
