def Load_model(name, args):
    ...
    def scatter_distribution(src: torch.Tensor, index: torch.Tensor, dim: int = -1,
        ...
        skew = src_minus_mean * src_minus_mean * src_minus_mean / (var.gather(dim,index) + 1e-7)**(1.5)
        kurtosis = (src_minus_mean * src_minus_mean * src_minus_mean * src_minus_mean) / (var * var + 1e-7).gather(dim,index)
        ...
        def __init__(self):
            ...
            class NodeModel(torch.nn.Module):
                def __init__(self,hcs,act):
                    ...
                    self.hcs = hcs
                    self.act = act
                    self.FeaStConv = FeaStConv( (2 + N_scatter_feats)*self.hcs, (2 + N_scatter_feats)*self.hcs, 1, False)
                    self.decoder = torch.nn.Linear( (2 + N_scatter_feats)*self.hcs, self.hcs )

                def forward(self, x, edge_index, edge_attr, u, batch):
                    row, col = edge_index
                    x = torch.cat([x,scatter_distribution(edge_attr, col, dim=0),u[batch]],dim=1)
                    x = self.act(self.FeaStConv( x, edge_index))
                    x = self.act(self.decoder( x ))
                    return x
