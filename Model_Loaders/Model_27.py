        ...
        def __init__(self):
            ...
            class GRUConv(torch.nn.Module):
                def __init__(self,hcs = self.hcs, act = self.act):
                    ...
                    self.lin_CoC_msg = MLP([self.hcs, self.hcs],clean_out = True) #version a
                ...
                def forward(self, x, CoC, h, batch):
                    ...
                    msg = self.lin_CoC_msg( self.scatter_norm(h, batch, CoC) ) #version a
            ...
            class scatter_att(torch.nn.Module): #Version a
                ...
                def forward(self, x, batch, CoC):
                    att = scatter_softmax(self.att_lin(torch.cat([x,CoC[batch]],dim=1)),batch,dim=0)
            ...
            self.scatter_norm2 = scatter_att(self.hcs*(2+N_metalayers)/2) #version a

            # self.decoder = MLP([(1+N_scatter_feats)*self.hcs,3*self.hcs,self.hcs,N_outputs],clean_out=True)
            self.decoder = MLP([(2+N_metalayers)*self.hcs,3*self.hcs,self.hcs,N_outputs],clean_out=True) #version a
